...
const rules = JSON.parse(
  fs.readFileSync(path.join(__dirname, "..", "data", "mer-rules.json"), "utf8")
);
...
function findEducationEquivalency(qualification) {
  ...
  if (!text) return null;
  return rules.educationEquivalencies.find((entry) =>
...
function evaluateAcademic(extracted) {
  ...
  if (subjectsPassed !== null && subjectsPassed >= 3 && normalize(extracted.highestQualification).includes("o-level")) {
  ...
  const hasFormalQualification = normalize(extracted.highestQualification) !== "" || yearsFormalEducation !== null;
  if (!hasFormalQualification && workExperienceYears !== null && workExperienceYears >= 5) {
    reasons.push(`Applicant reports ${workExperienceYears} years of work experience without formal qualification evidence.`);
...
function evaluateEnglish(extracted) {
  ...
  const benchmarks = rules.englishBenchmarks;
  ...
  if (source.includes("ielts") && parsedScore !== null) {
    ...
    flags.push("English requirement not met, recommend UCC English Placement Test or English Course pathway.");
  ...
  if ((source.includes("ucc placement") || source.includes("placement test")) && parsedScore !== null) {
    if (parsedScore >= benchmarks.ucc_placement_test.minimum) {
      reasons.push(`UCC Placement Test score ${parsedScore} meets the minimum of ${benchmarks.ucc_placement_test.minimum}.`);
...
function listMissingEvidence(extracted, files) {
  ...
  if (!normalize(extracted.englishQualification) && extracted.englishScore == null && !normalize(extracted.englishGrade)) {
...
function deriveFileWarnings(files) {
  ...
  for (const file of files) {
    if (file.type === "application/pdf" && !normalize(file.textPreview)) {
      warnings.push(`PDF file "${file.name}" may require OCR or manual review because no text preview was available.`);
    }
...
function assessApplicant(payload) {
  const course = rules.courses[payload.applicant.courseId] || rules.courses["dbm-elearning"];
