...
function buildPrompt(payload) {
  const applicant = payload.applicant || {};
  const fileDescriptions = (payload.files || []).map((file, index) => {
    const preview = file.textPreview ? ` Preview text: ${file.textPreview}` : "";
    return `${index + 1}. ${file.name} (${file.type}, ${file.size} bytes).${preview}`;
  });
...
async function extractApplicantFacts(payload) {
  const apiKey = process.env.OPENAI_API_KEY;
  if (!apiKey) {
    return manualFallback(
      payload,
      "OPENAI_API_KEY is not set. AI extraction was skipped and manual fields were used where available."
    );
  }

  try {
    return await callOpenAI(payload, apiKey);
  } catch (error) {
    return manualFallback(
      payload,
