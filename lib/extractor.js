...
async function callOpenAI(payload, apiKey, model, temperature) {
  ...
  const bodyObj = {
    ...
    input: [
      {
        role: "system",
        content: [{ type: "input_text", text: "You extract admissions evidence into structured JSON. Never add markdown fences." }]
      },
      { role: "user", content }
    ],
    text: {
      format: {
        ...
        schema: {
          ...
          properties: {
            ...
            passedSubjects: { type: "array", items: { type: "string" } },
          ...
          required: [
            "applicantName", "age", "dateOfBirth", "country", "educationSystem",
            "highestQualification", "yearsFormalEducation", "subjectsPassedCount",
            "passedSubjects", "englishQualification", "englishScore", "englishGrade",
            "workExperienceYears", "missingEvidence", "explanation"
          ]
        }
      }
    }
  ...
  if (temperature !== undefined && temperature !== null) {
    bodyObj.temperature = Number(temperature);
  }
...
async function extractApplicantFacts(payload, options = {}) {
  ...
  try {
    return await callOpenAI(enrichedPayload, apiKey, model, temperature);
  } catch (error) {
    return manualFallback(
      enrichedPayload,
